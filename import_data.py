...
from coffice.models import Spot
import csv

with open('coffice_spot.csv') as csvfile:
  reader = csv.DictReader(csvfile, delimiter=';')
  for row in reader:
    spot = Spot(id=row['id'], city=row['city'], name=row['name'], longitude=row['longitude'],
      latitude=row['latitude'], download_speed=row['download_speed'], upload_speed=row['upload_speed'], speed_test_link=row['speed_test_link'],
